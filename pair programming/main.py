...
def check_valid_string(input_text):
    ...
    if (valid_str.match(input_text) and "() ->" not in input_text) or len(input_text.split("\"")) > 4 or (not input_text.split(",")[-1].strip().isalnum() and len(input_text.split(","))>1 and "() ->" not in input_text) or not "\"" in input_text:
        return True
    else:
        return False
